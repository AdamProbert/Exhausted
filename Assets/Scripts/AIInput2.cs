...
public class AIInput2 : MonoBehaviour
{
    ...
    private UnityStandardAssets.Vehicles.Car.CarController m_CarController;    // Reference to actual car controller we are controlling
    ...
    Vector3 endDestination = Vector3.zero;
    RaycastHit m_HitInfo = new RaycastHit();
    [SerializeField] float secondsPerPathUpdate;
    ...
    private void UpdatePath()
    {
        ...
        {
            endDestination = player.position;
        }

        // + transform.forward * (m_Rigidbody.velocity.magnitude
        NavMesh.CalculatePath(
            transform.position, // Factor momentum into pathing
            endDestination,
            NavMesh.AllAreas,
            m_Path
        );
    ...
    private void Update() 
    {
        if(Time.time > lastPathUpdate+secondsPerPathUpdate)
        {
            UpdatePath();
        }
    }
    ...
    private void FixedUpdate()
    {
        ...
        {
            m_Target.position = destination;

            // Try to predict corner
            if(m_Path.corners.Length > pathIter+2)
                m_Target.LookAt(m_Path.corners[pathIter+2]);
            else if(m_Path.corners.Length > pathIter+1)
                m_Target.LookAt(m_Path.corners[pathIter+1]);
        }
